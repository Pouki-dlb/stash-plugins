...
declare global {
  ...
  /**
   * Props du composant FolderSelect de Stash.
   * Source : ui/v2.5/src/components/Shared/FolderSelect/FolderSelect.tsx
   *
   * Déclarée globalement pour rester utilisable depuis src/ sans import.
   */
  interface IFolderSelectProps {
    ...
    /**
     * Bouton "+" fourni par la fenêtre parente. Son onClick est une closure sur
     * l'état interne de cette fenêtre : il ajoute le répertoire *courant* à la
     * sélection, sans accepter d'argument.
     */
    appendButton?: React.ReactElement<{ onClick?: () => void }>;
    collapsible?: boolean;
    quotePath?: boolean;
    hideError?: boolean;
  }
...
interface IPluginApi {
  ...
  libraries: {
    ...
    FontAwesomeSolid: {
      ...
      faThumbtack: IconDefinition;
      faVenus: IconDefinition;
    };
    /**
     * Exposé par pluginApi.tsx. On l'utilise plutôt que
     * PluginApi.components.Icon : les bibliothèques sont disponibles dès le
     * chargement, alors que le registre de composants peut être incomplet
     * après un rechargement forcé (stash#5479).
     */
    ReactFontAwesome: {
      FontAwesomeIcon: React.FC<{
        icon: IconDefinition;
        className?: string;
      }>;
    };
...
interface PatchableComponentsAfter {
  ...
  (
    ...
    fn: (props: ISceneCardProps) => React.JSX.Element[]
  ): void;
}
